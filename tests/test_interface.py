...
def test_cli_incorrect_no_option(capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "error: one of the arguments -t/--target_id -d/--disease_id is required"
        in captured.err
    )
...
@pytest.mark.parametrize("target_id, disease_id", [("exists", None), (None, "exists")])
def test_run_analysis(target_id, disease_id):
    ...
    with patch(
        "association_analysis.interface.query_data_into_df_with_columns",
        return_value=expected_df,
    ), patch(
        "association_analysis.interface.statistics_with_columns",
