...
@pytest.mark.parametrize(
    ...
    [
        (
            "{0}{1}".format(api_url_base, endpoint),
            {"target": ["ENSG00000197386"], "size": 1},
            (200, 1, "application/json"),
        ),
        (
            "{0}{1}".format(api_url_base, endpoint),
            {"disease": ["Orphanet_399"], "size": 1},
            (200, 1, "application/json"),
        ),
        (
            "{0}{1}".format(api_url_base, endpoint),
            {"target": ["Orphanet_oh_no"], "size": 1},
            (200, 0, "application/json"),
        ),
    ],
)
...
def test_post_single_payload_when_response_is_ok():
    ...
    data_dict = query_api.post_single_payload(
        "https://www.host.com/fake/base/url", "/fake/endpoint", "s"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"data": "OMG", "next": "here"},
            ({"data": "OMG", "next": "here"}, {"data": "OMG", "next": "here"}),
        ),
    ],
)
def test_post_payload_generator(data_dict, expected):
    with patch(
        "association_analysis.query_api.post_single_payload", return_value=data_dict
    ):
        g = query_api.post_payload_generator(
            "/fake/base/url", "/fake/endpoint", {"dummy": "payload"}
        )
...
def test_query_data_into_df_with_columns():
    ...
    with patch(
        ...
        return_value=[
            {"data": {"column_i_want": 1, "no": 42}},
            {"data": {"column_i_want": 2, "yes": 42}},
        ],
    ):
        data_df = query_api.query_data_into_df_with_columns(
            "/url/base", "/endpoint", {"some": "payload"}, ["column_i_want"]
        )
